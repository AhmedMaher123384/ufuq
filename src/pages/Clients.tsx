...
import sallaLogo from '../assets/sallalogo.webp';
...
const Clients: React.FC = () => {
  ...
  // اختيار شعار محلي مناسب للعميل
  const resolveClientLogo = (client: Client): string => {
    const name = (client.name || '').toLowerCase();
    const site = (client.website || '').toLowerCase();

    // إذا تم تمرير مسار محلي بالفعل من الـ API أو الموك
    if (client.logo && client.logo.startsWith('/assets/')) {
      return client.logo;
    }

    // تعيين شعارات محلية بناءً على الاسم أو الموقع
    if (site.includes('salla') || name.includes('salla') || name.includes('سلة')) {
      return sallaLogo;
    }

    if (site.includes('whatsapp') || name.includes('whatsapp') || name.includes('واتساب')) {
      return whatsappLogo;
    }

    // افتراضيًا استخدم شعار الموقع
    return logo;
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <Link to="/" className="flex items-center space-x-3 space-x-reverse">
              <img src={logo} alt=" UfuqDigital" className="h-8 w-auto" />
              <span className="text-xl font-bold text-gray-900"> UfuqDigital </span>
            </Link>
            <nav className="hidden md:flex items-center space-x-8 space-x-reverse">
              <Link to="/" className="text-gray-600 hover:text-gray-900 transition-colors">الرئيسية</Link>
              <Link to="/products" className="text-gray-600 hover:text-gray-900 transition-colors">الخدمات</Link>
                ...
                <div key={(client as any).id ?? `${client.name}-${client.website ?? ''}-${(client as any).createdAt ?? ''}-${idx}`} className="group bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 border border-gray-100">
                  <div className="p-8 relative">
                    ...
                    <div className="flex items-center justify-between mb-6">
                      ...
                      >
                        <img
                          src={buildImageUrl(resolveClientLogo(client))}
                          alt={client.name}
