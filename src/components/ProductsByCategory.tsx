...
const ProductsByCategory: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const categoryData = mockCategories.find((c) => c.id === catIdNum) || null;
      const productsData = mockProducts.filter((p) => p.categoryId === catIdNum);

      setCategory(categoryData as any);
      setProducts(productsData as any);

      dataCache.set(`category_${categoryId}`, { data: categoryData, timestamp: now });
      dataCache.set(`products_${categoryId}`, { data: productsData, timestamp: now });
