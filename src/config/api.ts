...
export const API_CONFIG = {
  ...
  production: {
    baseURL: 'https://UfuqDigital-b.onrender.com', // UfuqDigital backend on Render
  }
};
...
export const buildImageUrl = (imagePath: string): string => {
  if (!imagePath) return placeholderProduct;
  const path = imagePath.trim();

  // External or embedded images
  if (path.startsWith('http')) return path;
  if (path.startsWith('data:image/')) return path;

  // Frontend static assets (served by Vite) 
  const fePrefixes = ['/assets/', 'assets/', '/src/assets/', 'src/assets/', '/public/', 'public/'];
  for (const fe of fePrefixes) {
  ...
  // Backend static directories
  const bePrefixes = ['/images/', 'images/', '/uploads/', 'uploads/', '/files/', 'files/', '/static/', 'static/'];
  for (const be of bePrefixes) {
    if (path.startsWith(be)) {
      return `${baseUrl}${normalize(path)}`;
    }
  }

  // Other absolute paths → keep as-is (likely frontend absolute)
  if (path.startsWith('/')) {
    return path;
  }

  // Default fallback: treat as image under /images on backend
